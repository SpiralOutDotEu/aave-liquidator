...
const theGraphURL_v3_mainnet =
  "https://api.thegraph.com/subgraphs/name/aave/protocol-v3";
const theGraphURL_v3 = theGraphURL_v3_polygon;
...
export const fetchV3UnhealthyLoans = async (
  ...
  try {
    const res = await fetch(theGraphURL_v3, {
      method: "POST",
    ...
    //   // if (total_loans > 0)
    //   //   console.log(
    //   //     `Records:${total_loans} Unhealthy:${unhealthyLoans.length}`
    //   //   );
    //   return res.data;
    // });
...
function parseUsers(payload) {
  ...
  loans = loans.filter((loan) => {
    if (!TOKEN_LIST[loan.max_borrowedSymbol]) {
      console.warn(
        `Warning: Symbol not found in TOKEN_LIST: ${loan.max_borrowedSymbol}`
      );
      return false; // Skip this loan as the token is not found
    }
    if (!TOKEN_LIST[loan.max_collateralSymbol]) {
      console.warn(
        `Warning: Symbol not found in TOKEN_LIST: ${loan.max_collateralSymbol}`
      );
      return false; // Skip this loan as the token is not found
    }
    console.log(
      "TOKEN: ",
      loan.max_borrowedSymbol,
      " - ",
      loan.max_collateralSymbol
    );
    return (
...
async function liquidationProfit(loan) {
  //flash loan fee
  const flashLoanAmount = percentBigInt(
    BigInt(loan.max_borrowedPrincipal),
    allowedLiquidation
  );
  ...
  var flashLoanAmountInEth_plusBonus = percentBigInt(
    flashLoanAmountInEth,
    loan.max_collateralBonus
  ); //add the bonus
  var collateralTokensFromPayout =
    (flashLoanAmountInEth_plusBonus *
      BigInt(10 ** TOKEN_LIST[loan.max_collateralSymbol].decimals)) /
  ...
  var fromTokenAmount = new TokenAmount(
    TOKEN_LIST[loan.max_collateralSymbol],
    collateralTokensFromPayout
  ); // this is the number of coins to trade (should have many 0's)
  var bestTrade = await useTradeExactIn(
    fromTokenAmount,
    TOKEN_LIST[loan.max_borrowedSymbol]
  );

  var minimumTokensAfterSwap = bestTrade
    ? (BigInt(bestTrade.outputAmount.numerator) *
        BigInt(10 ** TOKEN_LIST[loan.max_borrowedSymbol].decimals)) /
  ...
  if (profitInEthAfterGas > profit_threshold) {
    console.log("-------------------------------");
    console.log(`user_ID:${loan.user_id}`);
    console.log(`HealthFactor ${loan.healthFactor.toFixed(2)}`);
    console.log(
      `flashLoanAmount ${flashLoanAmount} ${loan.max_borrowedSymbol}`
    );
    console.log(`flashLoanAmount converted to eth ${flashLoanAmountInEth}`);
    console.log(
      `flashLoanAmount converted to eth plus bonus ${flashLoanAmountInEth_plusBonus}`
    );
    console.log(
      `payout in collateral Tokens ${collateralTokensFromPayout} ${loan.max_collateralSymbol}`
    );
    console.log(
      `${loan.max_borrowedSymbol} received from swap ${minimumTokensAfterSwap} ${loan.max_borrowedSymbol}`
    );
    bestTrade ? showPath(bestTrade) : console.log("no path");
    console.log(`flashLoanPlusCost ${flashLoanPlusCost}`);
    console.log(`gasFee ${gasFee}`);
    console.log(
      `profitInEthAfterGas ${Number(profitInEthAfterGas) / 10 ** 18}eth`
    );
  }
